...
# vía el MÉTODO VERIFICADO: override de player.entity.json (el arma es un hueso del rig del jugador -> cae
# perfecta en la mano, 1ª y 3ª persona, sin adivinar coordenadas). Detección de NUESTRO item por TAG.
#
# Usa la base COMPLETA de AG2 (cero referencias colgantes -> seguro para jugadores SIN arma). Edit quirúrgico:
# nuestro 'nemeles:rifle' (que Geyser ve como item con tag 'ak47') dispara variable.ak47.
#
# Fuente: ~/Downloads/_ag2rp (AG2 RP ya extraído). Destino: resourcepack/NemelesRP_Bedrock.
import json, os, shutil

AG2 = os.path.expanduser(r"~\Downloads\_ag2rp")
B = "NemelesRP_Bedrock"
# Armas de AG2 que activamos (cada una se dispara por su TAG, que añadimos al item via Geyser):
...
def edit_player_entity():
    ...
    sc = d.get("scripts", {})
    def add_tag(line, g):
        # añade '|| tag(g)' a CUALQUIER comparacion get_equipped_item_name(...)=='g' de esa linea
...
if __name__ == "__main__":
    ...
    # validar que player.entity es JSON válido
